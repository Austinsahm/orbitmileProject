...
export default function Component() {
  ...
  const signInWithGoogle = async () => {
    ...
    if (error || !url) {
      return redirect("/login?message=Could not authenticate user with Google");
    }
        ...
        <form className="space-y-6" method="post" action={signInWithEmail}>
          ...
          <div>
            <Button
              ...
              formAction={signUp}
              variant="outline"
              className="relative flex w-full justify-center rounded-md  py-2 px-4 text-sm font-medium text-foreground shadow-sm hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 hover:text-primary-foreground"
            >
              Sign Up
            </Button>
          </div>
